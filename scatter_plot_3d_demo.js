...
//            function(row) { return [1, y(row[axisKeys[1]] * 5)]; })
  ...
  function initializeDataGrid() {
    var csvfile = "data/pilot4_rightOfLegs_top_1_bodyTracking.csv";
    d3.csv(csvfile, function(csv){
    	...
    	d3.select('#slider1').call(d3.slider().axis(true).min(0).max(Time.length-1).step(.5).value(frame)
		...
		d3.select('#buttons').append('input')
          .attr('type', 'image')
          .attr('src', 'blue-play-button-md.png')
          .attr('class', 'play')
  ...
  function toggle(el){
    ...
    {
        el.src='pause+button.jpg';
        el.className="pause";
        return true;
    }
    else if(el.className=="pause")
    {
        el.src='blue-play-button-md.png';
        el.className="play";
        return false;
    }
}
